...
const History = () => {
  ...
  const filtered = useMemo(() => {
    return history.filter((row) => {
      ...
      const matchesYear =
        yearFilter === "All" || `20${month.split("-")[1]}` === yearFilter;

      const matchesMonth =
        monthFilter === "All" || month.startsWith(monthFilter);
  ...
  const exportFiltered = () => {
    ...
    downloadCsv("history_filtered.csv", rows, [
      "Month",
      "Consumption_KWh",
      "Bill_Amount",
    ]);
    ...
    <PageWrapper className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200/70 dark:border-white/10 bg-white dark:bg-[#111827] p-4 flex flex-wrap gap-3 items-center">
        <input
          ...
          className="rounded-xl border border-slate-300/70 dark:border-white/10 px-3 py-2 text-sm bg-transparent"
        />
        ...
        <div className="ml-auto">
          <ExportButton label="Export CSV" onClick={exportFiltered} />
        </div>
      </div>
              ...
              {historyLoading ? (
                ...
                paginated.map((row, index) => (
                  <tr
                    ...
                    } hover:bg-emerald-50 dark:hover:bg-emerald-900/15 transition`}
                  >
      ...
      <div className="flex items-center justify-end gap-2 text-sm">
        <button
          ...
          className="px-3 py-1.5 rounded-lg border border-slate-300/70 dark:border-white/10"
        >
          Prev
        </button>
